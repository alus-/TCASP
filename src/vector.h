...
struct Vector: Point {
	Vector(const short &_x, const short &_y): Point(_x,_y) {};
	bool iszero() const;
	Vector Perpendicular() const;
};

struct VectorAC: Point {
	float mod;
	float modsquared;
	float bearing;

	VectorAC(const Vector &v): Point(v.x,v.y) {
		updateangular();
	}

	VectorAC(const short &_x, const short &_y): Point(_x,_y) {
		updateangular();
	}

	VectorAC(const float &_mod, const float &_bea): Point(0,0), mod(_mod), bearing(_bea) {
		updatecartesian();
